...
def highlight_items(line):
    ...
    identifiersPattern = r'(?=^#*)(([a-zA-Z_]+)[0-9]*)'
    ...
    for i in range(0, len(parts),2):# Itera sobre las partes de la línea
        parts[i] = re.sub(operatorPattern, r'#OPERATORa__\g<0>#OPERATORb__', parts[i])# Reemplaza los operadores con el texto #OPERATOR# al inicio y al final incluyendo el mismo operador
    line = ''.join(parts)# Une las partes de la línea con las modificaciones y las partes de las comillas
